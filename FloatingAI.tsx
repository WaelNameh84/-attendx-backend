...
import { Bot, Send, X, Loader2, Mic, MicOff, GripVertical, User, Sparkles, Zap, Star, Heart, MessageCircle, Cpu, Wand2, Rocket, Brain, KeyRound, Eye, EyeOff, CheckCircle2, XCircle } from "lucide-react";
...
export default function FloatingAI() {
  ...
  const { assistantName, assistantPersonality, ttsEnabled, wakeWord, aiButtonIcon, aiButtonShape, aiButtonColor, aiButtonCustomColor } = useSettings();
  ...
  useEffect(() => {
    ...
    if (open && !localStorage.getItem("gemini_api_key")) {
      authFetch("/api/settings/my-ai-key")
  ...
  const handleQuickSaveKey = useCallback(async () => {
    ...
    try {
      const res = await authFetch("/api/ai/verify-key", {
        method: "POST",
    ...
    <>
      ...
      <button
        ...
        style={{
          ...
          ...(open ? { background: "var(--muted)", color: "var(--foreground)" } : btnStyle),
        }}
        className={cn(
