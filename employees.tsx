...
import { Plus, Search, Trash2, User, Loader2, Pencil, KeyRound } from "lucide-react";
...
export default function EmployeesPage() {
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      const res = await authFetch(`/api/users/${resetPwUser.id}/password`, {
        method: "PUT",
      ...
      <div className="space-y-6 max-w-5xl">
        ...
        <div className="bg-card border border-card-border rounded-xl divide-y divide-border">
          {isLoading ? (
            ...
            <div key={user.id} className="px-5 py-4 flex items-center gap-4" data-testid={`row-employee-${user.id}`}>
              ...
              <Button variant="ghost" size="icon" className="text-muted-foreground hover:text-amber-500" title="إعادة تعيين كلمة المرور" onClick={() => openResetPw(user as UserRecord)}>
        ...
        <Dialog open={resetPwOpen} onOpenChange={setResetPwOpen}>
          <DialogContent className="max-w-sm">
            <DialogHeader><DialogTitle>إعادة تعيين كلمة المرور: {resetPwUser?.name}</DialogTitle></DialogHeader>
            <form onSubmit={handleResetPassword} className="space-y-4 mt-2">
              ...
              <Button type="submit" className="w-full" disabled={resetPwLoading}>
                {resetPwLoading ? <Loader2 className="w-4 h-4 animate-spin me-2" /> : <KeyRound className="w-4 h-4 me-2" />}
                تغيير كلمة المرور
              </Button>
            </form>
          </DialogContent>
