...
function MessagesContent() {
  ...
  const { data: inbox = [], isLoading: loadingInbox } = useQuery<Msg[]>({
    ...
    queryFn: async () => {
      const r = await authFetch("/api/messages/inbox");
      return r.ok ? r.json() : [];
    },
  ...
  const { data: sent = [], isLoading: loadingSent } = useQuery<Msg[]>({
    ...
    queryFn: async () => {
      const r = await authFetch("/api/messages/sent");
      return r.ok ? r.json() : [];
    },
  });
  ...
  const { data: thread } = useQuery<{ parent: Msg; replies: Msg[] }>({
    ...
    queryFn: async () => {
      const r = await authFetch(`/api/messages/${selected!.id}/thread`);
  ...
  async function openMsg(msg: Msg) {
    ...
    if (!msg.isRead && tab === "inbox") {
      await authFetch(`/api/messages/${msg.id}/read`, { method: "PATCH" });
        ...
        <div className="rounded-2xl border border-border bg-card overflow-hidden">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-muted-foreground gap-2">
              ...
              <p className="text-sm">{tab === "inbox" ? "لا توجد رسائل في صندوق الوارد" : "لا توجد رسائل مُرسَلة"}</p>
            </div>
          ) : (
