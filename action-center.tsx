...
import { authFetch, authHeaders } from "@/lib/api-url";
...
function ActionCenterContent() {
  ...
  const { data: pendingJustifications = [], refetch: refetchJustifications } = useQuery<any[]>({
    ...
    queryFn: async () => {
      const res = await authFetch("/api/attendance/justifications");
      if (!res.ok) return [];
  ...
  async function clearAllNotifications() {
    await authFetch("/api/notifications/clear-all", { method: "POST" });
    invalidate();
  ...
  async function handleLeaveAction(notifId: number, leaveId: number, action: "approved" | "rejected") {
    ...
    try {
      const res = await authFetch(`/api/leave/${leaveId}`, {
        method: "PATCH",
  ...
  async function handleReviewJustification(justId: number, decision: "approved" | "rejected") {
    ...
    try {
      const res = await authFetch(`/api/attendance/justifications/${justId}`, {
        method: "PATCH",
                ...
                <div key={n.id} className={cn("px-5 py-4 flex gap-4", n.status === "unread" && "bg-primary/[0.03]")}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex gap-2 mt-3 flex-wrap">
                      ...
                      {(n.type === "LATE_CHECKIN" || n.type === "SYSTEM_ALERT") && (
