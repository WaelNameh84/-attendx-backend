...
import {
  ...
  BarChart3, ChevronDown, ChevronUp, CalendarDays,
} from "lucide-react";
      ...
      if (isOnline) {
        ...
        if (docPath) {
          ...
          toast({
            ...
            description: "The document could not be uploaded. Your request will be submitted without an attachment.",
            variant: "destructive",
          });
          /* Do NOT return — continue submitting without the document */
        }
      }
      /* If offline, skip upload silently and continue to queue below */
    } else if (objectPath) {
    ...
    <Layout>
      <div className="space-y-5 max-w-4xl">
                ...
                <form onSubmit={handleCreate} className="space-y-4 mt-2">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      ...
                      <Input type="date" value={form.startDate} onChange={e => setForm(f => ({ ...f, startDate: e.target.value }))} required data-testid="input-leave-start" />
                    </div>
                    <div className="space-y-1">
                      <Label>{t("end_date")}</Label>
                      <Input type="date" value={form.endDate} onChange={e => setForm(f => ({ ...f, endDate: e.target.value }))} required data-testid="input-leave-end" />
                    </div>
                  </div>
        ...
        <div className="bg-card border border-card-border rounded-xl divide-y divide-border">
            ...
            <div key={leave.id} className="px-5 py-4 flex items-start gap-4" data-testid={`row-leave-${leave.id}`}>
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                {isAdmin && leave.status === "pending" && (
                  <>
                    <Button size="icon" variant="ghost" className="text-green-600 hover:text-green-600 w-8 h-8" onClick={() => handleStatus(leave.id, "approved")} data-testid={`button-approve-leave-${leave.id}`}><Check className="w-4 h-4" /></Button>
          ...
          {leaves?.length === 0 && (
            <p className="px-5 py-8 text-center text-sm text-muted-foreground">{t("no_leave_requests")}</p>
          )}
        </div>
