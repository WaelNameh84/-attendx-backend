...
export default function LoginPage() {
  ...
  useEffect(() => {
    // Clear stale token from localStorage only — do not call clearAuth()
    // because that wipes the module-level authTokenGetter which breaks
    // authenticated requests after the user logs back in.
  ...
  const runBiometricSequence = async () => {
    ...
    if (!emailValid || !password) {
      toast({ title: t("biometric_fill_credentials") || "أدخل البريد الإلكتروني وكلمة المرور أولاً", variant: "destructive" });
      return;
    }
    setBioStep("place");
    await delay(1000);
